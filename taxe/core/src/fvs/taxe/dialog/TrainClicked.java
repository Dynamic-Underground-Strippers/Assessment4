...
public class TrainClicked extends ClickListener {
    ...
    public void clicked(InputEvent event, float x, float y) {
        ...
        if (!train.isOwnedBy(currentPlayer)) {
            context.getSideBarController().displayFlashMessage("Opponent's " + train.getName() + ". Speed: " + train.getSpeed(), Color.RED, 2);
            return;
        }

        if (train.getFinalDestination() == null) {
            context.getSideBarController().displayFlashMessage("Your " + train.getName() + ". Speed: " + train.getSpeed(), Color.BLACK, 2);
        } else {
            context.getSideBarController().displayFlashMessage("Your " + train.getName() + ". Speed: " + train.getSpeed() + ". Destination: " + train.getFinalDestination().getName(), Color.BLACK, 2);
        }
